function StockDisplay(props) {
    ...
    const colour = priceChange > 0 ? "#137333" : (priceChange < 0 ? "#a50e0e" : "#3c4043");
    const backgroundColour = priceChange > 0 ? "#e8f4eb" : (priceChange < 0 ? "#fce8e6" : "#e8eaed");
    ...
    if (priceChange > 0) {
        arrow = <svg width="16" height="16" viewBox="0 0 24 24" focusable="false" className={`fill-[${colour}]`}><path d="M4 12l1.41 1.41L11 7.83V20h2V7.83l5.58 5.59L20 12l-8-8-8 8z"></path></svg>;
    } else if (priceChange < 0) {
        arrow = <svg focusable="false" width="16" height="16" viewBox="0 0 24 24" className={`fill-[${colour}]`}><path d="M20 12l-1.41-1.41L13 16.17V4h-2v12.17l-5.58-5.59L4 12l8 8 8-8z"></path></svg>;
    } else {
        arrow = null;
    }
                ...
                <div className="flex justify-between items-center bg-white px-6 py-5 rounded-[10px] w-[60vw] hover:bg-neutral-100 hover:cursor-pointer">
                    ...
                    <div className="flex justify-end items-center font-medium">
                        ...
                        <div className="w-[96px] flex justify-end items-center">
                            <div className={`text-[${colour}]`}>{sign}${Math.abs(priceChange).toFixed(2)}</div>
                        </div>
                        <div className="w-[116px] flex justify-end items-center">
                            <div className={`flex justify-evenly items-center p-1.5 rounded-lg text-[${colour}] bg-[${backgroundColour}]`}>
                                {arrow}
